...
class group_aggregation_bridge(nn.Module):
    def __init__(self, dim_xh, dim_xl, k_size=3, d_list=[1,2,5,7], use_uncertainty_guide=False):
        super().__init__()
        ...
        self.g0 = nn.Sequential(
            ...
            nn.Conv2d(group_input_channels, group_input_channels, kernel_size=3, stride=1, 
                      ...
                      dilation=d_list[0], groups=group_input_channels)
        )
        self.g1 = nn.Sequential(
            LayerNorm(normalized_shape=group_input_channels, data_format='channels_first'),
            nn.Conv2d(group_input_channels, group_input_channels, kernel_size=3, stride=1, 
                      padding=(k_size+(k_size-1)*(d_list[1]-1))//2, 
                      dilation=d_list[1], groups=group_input_channels)
        )
        self.g2 = nn.Sequential(
            LayerNorm(normalized_shape=group_input_channels, data_format='channels_first'),
            nn.Conv2d(group_input_channels, group_input_channels, kernel_size=3, stride=1, 
                      padding=(k_size+(k_size-1)*(d_list[2]-1))//2, 
                      dilation=d_list[2], groups=group_input_channels)
        )
        self.g3 = nn.Sequential(
            LayerNorm(normalized_shape=group_input_channels, data_format='channels_first'),
            nn.Conv2d(group_input_channels, group_input_channels, kernel_size=3, stride=1, 
                      padding=(k_size+(k_size-1)*(d_list[3]-1))//2, 
                      dilation=d_list[3], groups=group_input_channels)
        )
        self.tail_conv = nn.Sequential(
            LayerNorm(normalized_shape=dim_xl * 2 + 4 * guide_channels, data_format='channels_first'),
            nn.Conv2d(dim_xl * 2 + 4 * guide_channels, dim_xl, 1)
        )

    def _build_guide(self, mask):
        if self.use_uncertainty_guide:
            mask_prob = torch.sigmoid(mask)
            uncertainty = 1 - torch.abs(2 * mask_prob - 1)
...
class EGEUNet(nn.Module):
    ...
    def __init__(self, num_classes=1, input_channels=3, c_list=[8,16,24,32,48,64], bridge=True, gt_ds=True,
        ...
        if bridge: 
            self.GAB1 = group_aggregation_bridge(c_list[1], c_list[0], use_uncertainty_guide=use_uncertainty_guide)
            self.GAB2 = group_aggregation_bridge(c_list[2], c_list[1], use_uncertainty_guide=use_uncertainty_guide)
            self.GAB3 = group_aggregation_bridge(c_list[3], c_list[2], use_uncertainty_guide=use_uncertainty_guide)
            self.GAB4 = group_aggregation_bridge(c_list[4], c_list[3], use_uncertainty_guide=use_uncertainty_guide)
            self.GAB5 = group_aggregation_bridge(c_list[5], c_list[4], use_uncertainty_guide=use_uncertainty_guide)
            print('group_aggregation_bridge was used')
            if use_uncertainty_guide:
                print('uncertainty guide was used')
        if gt_ds:
